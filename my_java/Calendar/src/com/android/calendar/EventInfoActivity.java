...
public class EventInfoActivity extends Activity implements View.OnClickListener,
    ...
    private static final String[] EVENT_PROJECTION = new String[] {
        ...
        Events.ORGANIZER,            // 16
        Events.ORIGINAL_EVENT,       // 17: for repeat exception event
    };
    ...
    /**
     * Listens for intent broadcasts--handle {android.intent.action.TIMEZONE_CHANGED}
     * (2011/11/19)
     * Purpose: to update the local timeZone info. 
     * Show the local timeZone when the event time zone is different from the local time zone.
     */
    ...
    protected void onCreate(Bundle icicle) {
        ...
        String where = String.format(Locale.ENGLISH, CALENDARS_WHERE, mEventCursor.getLong(EVENT_INDEX_CALENDAR_ID));
        ...
        where = String.format(Locale.ENGLISH, ATTENDEES_WHERE, mEventId);
        ...
        if (hasAlarm) {
            ...
            where = String.format(Locale.ENGLISH, REMINDERS_WHERE, mEventId);
    ...
    public void onPause() {
        ...
        if (changed) {
            ...
            mOriginalMinutes = reminderMinutes;
        }
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (Utils.hasImportCalendarProvider(this)){
            item = menu.add(MENU_GROUP_SEND, MENU_SEND, 0, R.string.shareEvent);
    ...
    /**
     * Update local time zone. Show the local time zone when it is different from the event's time zone.
     * Now it is only be called by {@link #updateView()} & {@link #mIntentReceiver}.
     */
    private void updateLocalTimezone() {
        ...
        if (mTimezone != null && !mAllday &&
                (!TextUtils.equals(localTimezone, mTimezone) ||
            ...
            setVisibilityCommon(R.id.timezone_container, View.VISIBLE);
    ...
    private void updateView() {
        ...
        String rRule = mEventCursor.getString(EVENT_INDEX_RRULE);
        boolean hasAlarm = mEventCursor.getInt(EVENT_INDEX_HAS_ALARM) != 0;
        mTimezone = mEventCursor.getString(EVENT_INDEX_EVENT_TIMEZONE);
        mColor = mEventCursor.getInt(EVENT_INDEX_COLOR) & 0xbbffffff;

        View calBackground = findViewById(R.id.cal_background);
        if (Utils.hasImportCalendarProvider(this)){
            this.registerForContextMenu(calBackground);//register long press event.
            calBackground.setOnCreateContextMenuListener(this);
        }
        ...
        if(TextUtils.isEmpty(rRule)) {
            int indexOfOriginalEvent = mEventCursor.getColumnIndexOrThrow(Events.ORIGINAL_EVENT);
            String originalEvent = mEventCursor.getString(indexOfOriginalEvent);
            if(!TextUtils.isEmpty(originalEvent)) {
            	String selection = Events._SYNC_ID + "=?";
            	String[] args = { originalEvent };
            	originalCursor = managedQuery(Events.CONTENT_URI, EVENT_PROJECTION, selection, args, null);
            	if( originalCursor!=null && originalCursor.moveToFirst()){;
            	    originalRRule = originalCursor.getString(EVENT_INDEX_RRULE);//satic index: EVENT_PROJECTION
            		rRule = originalRRule;
            	}
            }
        }
        ...
        if (!TextUtils.isEmpty(rRule)) {
            ...
            if(!TextUtils.isEmpty(originalRRule)){
            	ImageView imageView  = (ImageView)findViewById(R.id.repeat_icon);
            	imageView.setImageResource(R.drawable.ic_repeat_dark_exception);
            }
        } else {
                    ...
                    textView.setOnKeyListener(new OnKeyListener() {
                        ...
                        public boolean onKey(View v, int keyCode, KeyEvent event) {
                            try {
                                ...
                                LogUtil.e(TAG, "EventInfoActivity--Activity Not Found Exception");
                                return true;
                            }
                        }
                    }
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
    	...
    	menu.setHeaderTitle(when.getText());
        menu.add(0, 0, 0, this.getText(R.string.shareEvent));
    ...
    public void sendEvent() {
        ...
        intent.putExtra(Intent.EXTRA_STREAM, Uri.parse(Utils.VCALENDAR_URI + mEventId));
