    ...
    private void init(MonthActivity activity, String timeZone) {
        ...
        mGestureDetector = new GestureDetector(getContext(), new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
                ...
                //Log.v(TAG, "After normaliz()ed, time(year, month, monthDay)=(" + time.year + "/" + time.month + "/" + time.monthDay + ")");
                //(time.year==2037 && time.month > 11) is to avoid the Time's bug that Time can not normalize successful sometime.
                if (time.year < 1970 || 2037 < time.year || (time.year==2037 && time.month > 11)) {
                    Toast.makeText(mParentActivity, 
                           getContext().getResources().getString(R.string.valid_date_range),
                           Toast.LENGTH_SHORT).show();
                    //If year is not in valid rang, just return.
                    return true;
                }
            ...
            public void onLongPress(MotionEvent e) {
                ...
                int startDay = Time.getJulianDay(millis, mViewCalendar.gmtoff);
                final long startMillis = getSelectedTimeInMillis();
                LogUtil.v(TAG, "startMillis=" + startMillis + ", startDay=" + startDay);
            ...
            public boolean onSingleTapUp(MotionEvent e) {
                if (mLaunchDayView) {
                    ...
                    /* Check valid range [1970, 2037] */
                    int startDay = Time.getJulianDay(millis, mViewCalendar.gmtoff);
                    LogUtil.v(TAG, "millis=" + millis + ", startDay=" + startDay);
