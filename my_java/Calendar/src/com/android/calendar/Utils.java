...
public class Utils {
    ...
    /**
     * Find out whether this event is the first one in series by the curTimeMills. 
     * If it is the first one, return true, otherwise false.
     * 
     * @param cursor The event's cursor.
     * @param originalCursor The event's original cursor
     * @param startTimeMills The event's start time( to Mills)
     * @return
     */
    public static boolean isFirstEventInSeries(Cursor cursor, Cursor originalCursor, long startTimeMills) {
        int dtStart = cursor.getColumnIndexOrThrow(Events.DTSTART);
        long start = cursor.getLong(dtStart);
        if(originalCursor!=null && originalCursor.moveToFirst()) {
        	start = originalCursor.getLong(dtStart);
        	//TODO: find the better method
        	//If the event is modified by a repeat event,just compare with its original event's date.
        	Time startTime = new Time();
        	startTime.set(start);
        	String startDate = startTime.format3339(true);
        	
        	startTime.set(startTimeMills);
        	String eventDate = startTime.format3339(true);
        	return startDate.equals(eventDate);
        }
        return start == startTimeMills;
    }
}
