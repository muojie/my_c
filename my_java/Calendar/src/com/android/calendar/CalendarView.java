...
public class CalendarView extends View
    ...
    private static int MAX_ALLDAY_EVENT_HEIGHT = 20;
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        LogUtil.v(TAG, "onKeyUp(keyCode=" + keyCode + "), mSelectionDay=" + mSelectionDay);
        mScrolling = false;
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_CENTER:
                ...
                if (mSelectionMode == SELECTION_SELECTED) {
                    if (mSelectionDay <= FIRST_CALENDAR_DAY || LAST_CALENDAR_DAY < mSelectionDay) {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        LogUtil.v(TAG, "onKeyDown(keyCode=" + keyCode + "), mSelectionMode=" + mSelectionMode);
        if (mSelectionMode == SELECTION_HIDDEN) {
        ...
        /* Check valid range [1970, 2037] */
        LogUtil.v(TAG, "onKeyDown(), selectionDay=" + selectionDay);
        if (keyCode == KeyEvent.KEYCODE_DPAD_CENTER) {
            if (selectionDay <= FIRST_CALENDAR_DAY || LAST_CALENDAR_DAY < selectionDay) {
        ...
        case KeyEvent.KEYCODE_DPAD_LEFT:
            ...
            if (mSelectedEvent == null) {
                ...
                /* Check valid range [1970, 2037] */
                if (selectionDay <= FIRST_CALENDAR_DAY) {
                    LogUtil.v(TAG, "OUT-OF-BOUND!");
                    Toast.makeText(mParentActivity, 
                           getContext().getResources().getString(R.string.valid_date_range),
                           Toast.LENGTH_SHORT).show();
        ...
        case KeyEvent.KEYCODE_DPAD_RIGHT:
            ...
            if (mSelectedEvent == null) {
                ...
                /* Check valid range [1970, 2037] */
                if (LAST_CALENDAR_DAY < selectionDay) {
                    LogUtil.v(TAG, "OUT-OF-BOUND!");
                    Toast.makeText(mParentActivity, 
                           getContext().getResources().getString(R.string.valid_date_range),
                           Toast.LENGTH_SHORT).show();
...
/*
            Time s = new Time();
            s.set(0, 0, 0, selectionDay - Time.EPOCH_JULIAN_DAY, 0, 1970);
            Log.v(TAG, "selectionDay(year, month, monthDay)=(" + s.year + "/" + s.month + "/" + s.monthDay + ")=" + s.toMillis(true));

            if (selectionDay < mFirstJulianDay) {
                Log.v(TAG, "selectionDay < mFirstJulianDay");
                Time t = new Time();
                t.set(0, 0, 0, mFirstJulianDay - Time.EPOCH_JULIAN_DAY, 0, 1970);
                Log.v(TAG, "mFirstJulianDay(year, month, monthDay)=(" + t.year + "/" + t.month + "/" + t.monthDay + ")=" + t.toMillis(true));
            }
            if (mLastJulianDay < selectionDay) {
                Log.v(TAG, "(selectionDay > mLastJulianDay");
                Time t = new Time();
                t.set(0, 0, 0, mLastJulianDay - Time.EPOCH_JULIAN_DAY, 0, 1970);
                Log.v(TAG, "mLastJulianDay(year, month, monthDay)=(" + t.year + "/" + t.month + "/" + t.monthDay + ")=" + t.toMillis(true));
            }
*/
    ...
    private void drawEvents(int date, int left, int top, Canvas canvas, Paint p) {
        ...
        if (date == mSelectionDay && !mSelectionAllDay && isFocused()
            ...
            computeNeighbors();
            if(mSelectedEvent != null ) {
                if(!events.contains(mSelectedEvent)) {
                    //update the event info when mSelectedEvent was too old.
                    for(Event event: events) {
                        if(mSelectedEvent.id == event.id) {
                            mSelectedEvent = event;
                            //update the event info in popup window.
                            mLastPopupEventID = -1;
                            break;
                        }
                    }
    ...
    private void updateEventDetails() {
        ...
        imageView.setVisibility((event.mRepeatStatus!=Event.REPEAT_NO) ? View.VISIBLE : View.GONE);
        if(event.mRepeatStatus == Event.REPEAT_EXCEPTION) {
        	imageView.setImageResource(R.drawable.ic_repeat_white_exception);
        } else {
        	imageView.setImageResource(R.drawable.ic_repeat_white);
        }
    ...
    void doSingleTapUp(MotionEvent ev) {
        ...
        LogUtil.v(TAG, "selectedDay=" + selectedDay);
        if (mSelectionDay <= FIRST_CALENDAR_DAY || LAST_CALENDAR_DAY < mSelectionDay) {
            Toast.makeText(mParentActivity, 
                   getContext().getResources().getString(R.string.valid_date_range),
                   Toast.LENGTH_SHORT).show();
    ...
    void doLongPress(MotionEvent ev) {
        ...
        LogUtil.v(TAG, "mSelectionDay=" + mSelectionDay);
        if (mSelectionDay <= FIRST_CALENDAR_DAY || LAST_CALENDAR_DAY < mSelectionDay) {
            Toast.makeText(mParentActivity,
                   getContext().getResources().getString(R.string.valid_date_range),
                   Toast.LENGTH_SHORT).show();
        ...
        if (mTouchMode == TOUCH_MODE_DOWN) {
            ...
            if (absDistanceX >= 2 * absDistanceY) {
                ...
                try {
                    ...
                    LogUtil.v(TAG, "doScroll()1 reach bound");
                    return;
                }
            } else {
            ...
            if (distanceX != 0) {
                ...
                if (direction != mPreviousDirection) {
                    ...
                    try {
                        ...
                        LogUtil.v(TAG, "doScroll()2 reach bound");
                        return;
                    }
    ...
    void doFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        if ((distanceX >= HORIZONTAL_SCROLL_THRESHOLD) && (distanceX > distanceY)) {
            ...
            try {
                switchForward = initNextView(deltaX);
            } catch (DayOutOfRangeException e) {
                LogUtil.v(TAG, "doFling() reach bound");
                Toast.makeText(mParentActivity, 
                       getContext().getResources().getString(R.string.valid_date_range),
                       Toast.LENGTH_SHORT).show();
                return;
    ...
    private boolean initNextView(int deltaX) throws DayOutOfRangeException {
        ...
        date.normalize(true /* ignore isDst */);
        /* for WeekView: 1969/11/21(invalid) 1969/11/28(first valid, partial), 1970/0/4(valid),
                         2037/11/27(last valid, partial), 2038/1/3(invalid).
           for DayView: 1969/11/31 (invaid), 1970/0/1(first valid), 2037/11/31(last valid), 2038/0/1(first vinalid).
           Common rule: invalid: (2037 < date.year) || (year < 1970 && (mBaseDate + mNumDays - 1 < 1970)).
         */
        if (date.year < 1970) {
            lastDay = date;
            lastDay.monthDay += mNumDays -1;
            lastDay.normalize(true /* ignore isDst */);
            LogUtil.v(TAG, "lastDay(year, month, monthDay)=(" + lastDay.year + "/" +  lastDay.month + "/" + lastDay.monthDay + ")");
        }

        if (2037 < date.year || (date.year < 1970 && lastDay.year < 1970)) {
           throw new DayOutOfRangeException("After adjust, date(year, month, monthDay)=(" +
                   date.year + "/" +  date.month + "/" + date.monthDay + "), mNumDays=" + mNumDays);
        }
        initView(view);
    ...
    private class DayOutOfRangeException extends IllegalAccessException {
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menuInfo) {
        ...
        if (mNumDays == 1) {
            ...
            if (numSelectedEvents >= 1) {
                ...
                if (Utils.hasImportCalendarProvider(getContext())){
                    item = menu.add(0, MenuHelper.MENU_EVENT_SEND, 0, getContext().getText(R.string.shareEvent));
                    item.setOnMenuItemClickListener(mContextMenuHandler);
            ...
            if (numSelectedEvents >= 1) {
                ...
                if (Utils.hasImportCalendarProvider(getContext())){
                    item = menu.add(0, MenuHelper.MENU_EVENT_SEND, 0, R.string.shareEvent);
                    item.setOnMenuItemClickListener(mContextMenuHandler);
                    item.setIcon(android.R.drawable.ic_menu_send);
                    item.setAlphabeticShortcut('s');
                }
    ...
    private class ContextMenuHandler implements MenuItem.OnMenuItemClickListener {
        public boolean onMenuItemClick(MenuItem item) {
            switch (item.getItemId()) {
                ...
                case MenuHelper.MENU_EVENT_SEND:{
                    sendEvent();// send event by bt & wifi
                }
                default: {
                    return false;
                }
    ...
    private void sendEvent() {
        if (mSelectedEvent != null){
            ...
            intent.putExtra(Intent.EXTRA_STREAM, Uri.parse(Utils.VCALENDAR_URI + mSelectedEvent.id));
    ...
    private static int getEventAccessLevel(Context context, Event e) {
        ...
        String where = String.format(Locale.ENGLISH, CALENDARS_WHERE, calId);
...
//                    + " mViewStartY: " + mViewStartY);
    ...
    /**
     * Recycle the bitmap and canvas used in the view.
     * the function will be called in CalendarActivity's onDestroy().
     */
